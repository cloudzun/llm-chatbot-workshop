...
TOOLS = [
    ...
    },
    {
        ...
        "function": {
            "name": "search_web",
            "description": (
                "搜索互联网获取知识、定义、概念解释、人物信息等。"
                "当用户询问具体知识点，且天气工具和新闻工具均不适用时调用。"
    ...
    if fn_name == "get_weather":
        ...
        with st.expander("🔍 搜索结果", expanded=True):
            if result.get("heading"):
                st.subheader(result["heading"])
            if result.get("answer"):
                st.info(result["answer"])
            if result.get("abstract"):
                st.write(result["abstract"])
            if result.get("definition"):
                st.write(f"**定义**：{result['definition']}")
            if result.get("relatedTopics"):
                st.markdown("**相关话题**")
                for topic in result["relatedTopics"]:
                    if topic.get("url"):
                        st.markdown(f"- [{topic['text']}]({topic['url']})")
                    else:
                        st.markdown(f"- {topic['text']}")
            if result.get("fallback_links"):
                st.markdown("未找到即时答案，可以通过以下链接搜索：")
                for link in result["fallback_links"]:
