...
| 新闻列表为空 | RSS 源暂时不可用 | 自动尝试备用源；`hnrss.org/frontpage` 通常最稳定 |
| BBC/Yahoo 源加载失败 | 国内网络访问受限 | 代码已替换为国内可用源（HN、OSChina、DW中文、36氪）|
...
> **🇨🇳 中文查询说明**：DDG 即时答案 API 对中文词条支持有限，中文查询常常无结果。代码已内置优化：当无结果时，自动返回必应 (cn.bing.com) 和百度的搜索链接，供用户直接点击搜索。
...
| 症状 | 原因 | 修复方法 |
|------|------|---------|
| DuckDuckGo 中文查询返回空 | DDG 即时答案 API 对中文词条支持有限 | 已内置中文优化：无结果时自动返回必应/百度搜索链接 |
| DuckDuckGo 英文查询也返回空 | 该词条 DDG 知识库无收录 | 换更通用的英文词（如 "quantum computing" 而非缩写）|
